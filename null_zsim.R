...
for (i in ntrials){
  ...
  for (j in asplit){#split t into groups by different fractions
    ...
    write.csv (orig_df, paste0("null_",i,"_",j,"_orig.csv")) #creates csv of simulation outputs
...
null_sim_df$ntrials <- factor(null_sim_df$ntrials)

#plot using sim_df output
null_mean_z_plot <- ggplot(null_sim_df, aes(asplit, sim_mean_z, col = ntrials)) + geom_line() + ggtitle("null_mean_z")
null_mean_plot <- ggplot(null_sim_df, aes(asplit, sim_mean, col = ntrials)) + geom_line() + ggtitle("null_mean")
